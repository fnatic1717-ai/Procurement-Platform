...
CREATE TYPE approval_decision AS ENUM ('pending','approved','rejected','returned','cancelled');
CREATE TYPE approval_instance_status AS ENUM ('active','approved','rejected','returned','withdrawn','cancelled','superseded');
CREATE TYPE idempotency_operation AS ENUM ('submit','approve','reject','return_request','assign_buyer');
...
CREATE TABLE purchase_request_approval_instances (
 ...
 submission_key text NOT NULL, status approval_instance_status NOT NULL DEFAULT 'active', created_at timestamptz NOT NULL DEFAULT now(), completed_at timestamptz, UNIQUE(tenant_id,id), UNIQUE(tenant_id,purchase_request_id,submission_key),
...
CREATE TABLE purchase_request_approval_steps (
 ...
 decision approval_decision NOT NULL DEFAULT 'pending', decision_by uuid, claimed_by uuid, decision_at timestamptz, comment text, version integer NOT NULL DEFAULT 1,
 created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), UNIQUE(tenant_id,id), UNIQUE(tenant_id,instance_id,step_number),
 FOREIGN KEY(tenant_id,instance_id) REFERENCES purchase_request_approval_instances(tenant_id,id), FOREIGN KEY(tenant_id,approver_user_id) REFERENCES tenant_memberships(tenant_id,user_id), FOREIGN KEY(tenant_id,approver_role_id) REFERENCES roles(tenant_id,id), FOREIGN KEY(tenant_id,decision_by) REFERENCES tenant_memberships(tenant_id,user_id), FOREIGN KEY(tenant_id,claimed_by) REFERENCES tenant_memberships(tenant_id,user_id));
...
CREATE TABLE idempotency_records (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, actor_id uuid NOT NULL, operation idempotency_operation NOT NULL, object_id uuid NOT NULL, key text NOT NULL, payload_hash text NOT NULL, response jsonb, created_at timestamptz NOT NULL DEFAULT now(), completed_at timestamptz,
 UNIQUE(tenant_id,actor_id,operation,object_id,key), FOREIGN KEY(tenant_id,actor_id) REFERENCES tenant_memberships(tenant_id,user_id));
CREATE INDEX idempotency_object_idx ON idempotency_records(tenant_id,object_id,operation);
...
('purchase_requests.cancel','Cancel eligible tenant purchase requests',false),
...
DO $$ DECLARE t text; BEGIN
 FOREACH t IN ARRAY ARRAY['purchase_requests','purchase_request_items','approval_policies','approval_policy_steps','purchase_request_approval_instances','purchase_request_approval_steps','procurement_intake_records','buyer_assignments','tenant_number_sequences','idempotency_records'] LOOP
...
CREATE OR REPLACE FUNCTION protect_approval_snapshot() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
 IF OLD.tenant_id<>NEW.tenant_id OR OLD.purchase_request_id<>NEW.purchase_request_id OR OLD.policy_id<>NEW.policy_id OR OLD.policy_version<>NEW.policy_version OR OLD.route_snapshot<>NEW.route_snapshot OR OLD.submission_key<>NEW.submission_key THEN RAISE EXCEPTION 'approval route snapshot fields are immutable'; END IF; RETURN NEW; END $$;
CREATE TRIGGER approval_instance_snapshot_immutable BEFORE UPDATE ON purchase_request_approval_instances FOR EACH ROW EXECUTE FUNCTION protect_approval_snapshot();
CREATE TRIGGER approval_instance_no_delete BEFORE DELETE ON purchase_request_approval_instances FOR EACH ROW EXECUTE FUNCTION prevent_audit_mutation();
